...
I performed a stepwise regression on the `birthweight_data`. The `step` function developed several regression models by adding and eliminating different predictors one at a time, and ultimately identifying the best model. My model includes `babysex`, `bhead`, `blength`, `delwt`, `fincome`, `gaweeks`, `mheight`, `mrace`, `parity`, `ppwt`, and `smoken` as signficant predictors of the baby's weight. My model is summarized in the code chunk below:
